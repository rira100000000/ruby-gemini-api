...
* Geminiモデルによるテキスト生成
* 会話履歴付きのチャット機能
* **思考モデル対応（Gemini 2.5/3.0）** - Thought Signatureの自動管理機能付き
...
### 思考モデルとThought Signatures

Gemini 2.5および3.0シリーズのモデルは「思考モード」をサポートしており、応答する前に問題について推論することができます。このライブラリは、Thought Signatureの自動管理機能を備えた思考モデルの完全サポートを提供します。
...
# 思考モードを有効化（最もシンプルな方法）
response = client.generate_content(
  "この問題を解いてください：ある店は在庫の45%を販売しました。220個の商品が残っている場合、元々何個の商品がありましたか？",
  model: "gemini-3-flash-preview",
  thinking_config: true  # デフォルト設定で思考を有効化
)
...
puts response.text  # 最終回答のみを返す（デフォルトで思考は除外される）
```

#### 異なる思考設定オプション

```ruby
# Gemini 3: thinking_levelを使用
response = client.generate_content(
  "1000ドルを5%の利率で3年間複利計算してください",
  model: "gemini-3-flash-preview",
  thinking_config: { thinking_level: "high", include_thoughts: true }
)

# Gemini 2.5: thinking_budgetを使用（関数呼び出しと併用）
tools = Gemini::ToolDefinition.new do
  function :calculate, description: "計算を実行" do
...
response = client.generate_content(
  "123 × 456 はいくつですか？",
  model: "gemini-2.5-flash",
  thinking_config: 8192,  # thinking_budgetの省略形
  tools: tools
)
...
#### 自動シグネチャ管理付きConversationクラス

`Conversation`クラスは、複数ターンに渡ってThought Signaturesを自動管理し、エラーを防ぎ、推論コンテキストを維持します：
...
conversation = Gemini::Conversation.new(
  client: client,
  model: "gemini-3-flash-preview",
  thinking_config: true  # 思考モードを有効化
)

# 最初のメッセージ
response1 = conversation.send_message("15 × 24 はいくつですか？")
puts response1.text  # 最終回答のみ
puts response1.thought_text if response1.has_thoughts?  # 思考プロセス（オプション）

# 会話を続ける - Thought Signaturesは自動的に保持される
response2 = conversation.send_message("その結果を3で割ってください")
puts response2.text

# シグネチャは自動的に処理されます - 手動管理は不要！
```
...
if response.has_thoughts?
  # 思考パーツを個別に取得
  puts "思考パーツ数: #{response.thought_parts.size}"
  puts "非思考パーツ数: #{response.non_thought_parts.size}"
  ...
  # [THOUGHT]プレフィックス付きの視覚的表現
  puts "\n[マーカー付き全内容]:"
  puts response.full_content
end

# Thought Signaturesの確認（Gemini 2.5で関数呼び出し使用時）
if response.has_thought_signatures?
  puts "シグネチャ数: #{response.thought_signatures.size}"
end
```

#### 重要な注意事項

**Gemini 2.5とGemini 3の違い：**

- **Gemini 2.5**:
  - Thought Signaturesを返すには関数呼び出し（toolsパラメータ）が必要
  - `thinking_budget`パラメータを使用（数値: 0-32768）
  - 明示的な`thought: true`フィールドなしでシグネチャを返す

- **Gemini 3**:
  - 明示的な`thought: true`フィールド付きで思考を返す
  - `thinking_level`パラメータを使用（文字列: "low", "medium", "high", "minimal"）
  - シグネチャを特定の位置に配置（最初の関数呼び出しパートまたは最後のパート）

**ベストプラクティス：**

1. 思考モデルとの複数ターン会話には`Conversation`クラスを使用
2. 最終回答には`response.text`にアクセス（思考は自動的に除外される）
3. 推論プロセスを検査するには`response.thought_text`を使用
4. Thought Signaturesはライブラリに自動処理させる
...
ruby demo/thinking_demo_ja.rb  # 日本語版

# 関数呼び出し付き思考
ruby demo/thinking_with_function_calling_demo.rb
ruby demo/thinking_with_function_calling_demo_ja.rb  # 日本語版

# Gemini 3関数呼び出しとシグネチャ検証
ruby demo/gemini3_function_calling_test_ja.rb
```

詳細については以下を参照：
- [Gemini 思考モデルドキュメント](https://ai.google.dev/gemini-api/docs/thinking?hl=ja)
- [Thought Signaturesドキュメント](https://ai.google.dev/gemini-api/docs/thought-signatures?hl=ja)
...
- `demo/thinking_demo.rb` / `demo/thinking_demo_ja.rb` - **思考モデル基本使用法**
- `demo/thinking_with_function_calling_demo.rb` / `demo/thinking_with_function_calling_demo_ja.rb` - **関数呼び出し付き思考モデル**
- `demo/gemini3_function_calling_test_ja.rb` - **Gemini 3関数呼び出しとシグネチャ検証**
...
# 関数呼び出し付き思考モデル
ruby demo/thinking_with_function_calling_demo_ja.rb
...
- `gemini-2.5-flash`（thinking budget対応）
- `gemini-2.5-pro`
- `gemini-3-flash-preview`（thinking level対応）

思考モデルについては、[思考モデルとThought Signatures](#思考モデルとthought-signatures)セクションを参照してください。
